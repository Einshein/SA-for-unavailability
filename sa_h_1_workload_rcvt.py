...
# add the workload-dependent recovery time
def costf_wl_rt(c_jj, r_jj, lambdas, mu, delta_jj, L_jj): #L_jj: number of functions which are protected by server j
    #numbner of fesible states
    if L_jj >= r_jj:
        num_feast = (r_jj*r_jj + r_jj) / 2 + (r_jj + 1)*(L_jj - r_jj + 1) + L_jj + 1
    else:
    ...
    #incoming sates    
        ...
        if all_states[k][3] >= 1:
            state = [all_states[k][0], all_states[k][1], all_states[k][2] + 1, all_states[k][3] - 1, all_states[k][4]]
            #rate = (all_states[k][2] + 1) * delta_jj 
            rate = (all_states[k][2] + 1) * delta_wl((all_states[k][2]+all_states[k][3])/r_jj,delta_jj)  
            indexs = all_states.index(state)
            A[k][indexs] = rate        
        #t8 workload
        if all_states[k][4] == 0: #q=0
    ...
    #outgoing sates        
        ...
        if all_states[k][2] >= 1:
            #rate = all_states[k][2] * delta_jj
            rate = all_states[k][2] * delta_wl((all_states[k][2]+all_states[k][3])/r_jj,delta_jj) 
            rates = rates - rate            
        #t17 worklload
        if all_states[k][4] == 1:
            rate = lambda_wl((all_states[k][2]+all_states[k][3])/r_jj)
...
#woroload-dependent recovery time of the backup server
def delta_wl(omega,delta_max):  # 1/100 < delta_max <1/10
    if omega<0 or omega>1:
        return
    delta_min = 1 / 100
    return delta_min+omega*(delta_max-delta_min)
...
#    costf(5, 3, 1 / 100, 1 / 10, 1 / 0.01, 2)
    ...
    for j in range(S):  #randomly set the list c_j and r_j
        c_jj = random.randint(1,capacity_range) # 16
    ...
    #     print("=====")      
        ...
        J_WL_RT = sa(F, S, c_j, r_j, lambdas, mu, delta_j, costf_wl_rt, T_i, T_t, cool)

        #Verify the maximum unavailability of the allocation J_ST under the workload-dependent(failure probability and recover time) scenario
        Q1=0 #maximum unavailability
        for j in range(S):
            q = costf_wl_rt(c_j[j], r_j[j], lambdas, mu, delta_j[j], J_ST[1][j])
            if q > Q1:
                Q1 = q
        #Verify the maximum unavailability of the allocation J_ST under the workload-dependent(failure probability and recover time) scenario   
        Q2=0 #maximum unavailability
        for j in range(S):
            q = costf_wl_rt(c_j[j], r_j[j], lambdas, mu, delta_j[j], J_WL[1][j])
            if q > Q2:
                Q2 = q
        ...
        adv_WlfpToBs=(Q1-Q2)/Q1   # reduction from wlfp to bs
        adv_WlfprtToBs=(Q1-J_WL_RT[0])/Q1  # reduction from wlfprt to bs
        #print(adv) 
        #return adv,Q,J_WL[0]
        return Q1, Q2, J_WL_RT[0], adv_WlfpToBs, adv_WlfprtToBs
